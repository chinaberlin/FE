...
if ( $('#colorpickerHolder').length > 0 ) {
	$('#colortoggle').click( function() { $(this).parent().toggleClass('closed'); });
    $('#colorpickerHolder').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			window.hexa = hex;
			$('.nav li a').attr('style', '');
			$('.bgcolor, .dark-bgcolor, span.service-icon-holder, .tara-portfolio-filters ul li a.selected, .tara-button.triangledown.selected , .ui-accordion .ui-accordion-header .ui-accordion-header-icon, .ui-accordion.bgfill h5.ui-accordion-header-active, .tara-tabs.ui-tabs.sidecontrols ul li.ui-tabs-active, .tara-tabs.ui-tabs ul li.ui-tabs-active a ').not('#colortoggle').not('.dark-bgcolor').css('backgroundColor', '#' + hex);
			$('.color, ul.nav li.active a').css('color', '#' + hex);
			$('.tara-button-year.selected').find('.triangle').css('border-color', '#'+hex+' transparent transparent');
			$('.border-color, .tara-button.transparent:hover, .tara-form input[type="text"]:focus, .tara-form input[type="password"]:focus, .tara-form input[type="email"]:focus, .tara-form textarea:focus ').css('borderColor', '#' + hex);
			$('.box .punchline .triangle.border-color').css('borderColor', 'transparent transparent #'+hex);
			$('.tara-button,  .tara-button.triangledown').not('#colortoggle').not('.transparent, .selected').hover( function() {
				$(this).css('backgroundColor', '#'+hex);
				$(this).find('.triangle').css('border-color', '#'+hex+' transparent transparent');
			}, function() {
				if( $(this).hasClass('tara-button-year') || !$(this).css('backgroundColor') == '#'+hex && !$(this).parent().parent().parent().hasClass('tara-portfolio-filters') ) {
					$(this).attr('style', '');
					$(this).find('.triangle').attr('style', '');
				}
			});
			$('.tara-portfolio-filters ul li a').hover(function() {
			...
			$('.tara-services-control a').hover( function() {
				$(this).find('.square').css('backgroundColor', '#'+hex);
			}, function() {
				$(this).find('.square').attr('style', '');
			});
			$('.tara-timeline-event').hover( function() {
				$(this).find('.tara-timeline-circle .circle').css('backgroundColor', '#'+hex);
			}, function() {
				$(this).find(' .tara-timeline-circle .circle').attr('style', '');
			});
			$('a').not('#colortoggle, .selected, .bgcolor').hover( function() {
				if ( !$(this).parent().parent().parent().hasClass('tara-portfolio-filters') ) {
					$(this).css('color', '#'+hex);
				}
			}, function() {
				if ( !$(this).parent().hasClass('active') || !$(this).parent().parent().parent().hasClass('tara-portfolio-filters') ) {
					$(this).attr('style', '');
				}
			});
			$('.tara-timeline-event').hover( function() {
				$(this).find('.tara-timeline-date, h5').css('color', '#'+hex);
			}, function() {
				$(this).find('.tara-timeline-date, h5').attr('style', '');
			});
			$(' ul.nav li a, .tara-button.transparent').hover( function() {
				$(this).css('color', '#'+hex);
			}, function() {
				if ( !$(this).parent().hasClass('active') || !$(this).css('backgroundColor') == '#'+hex ) {
					$(this).attr('style', '');
				}
			});
		}
	...
	var ps = 2; // replace 1 with 2 or 3 for the other parallax styles
	// ENABLE & DISABLE CUSTOM SCROLLBAR & SMOOTH SCROLLING
	var cs = 1; // USE 1 FOR ENABLE AND 0 FOR DISABLE ( for parallax style 3 we recommend to turn it on )
	...
	function drawTimer(percent, th){
		...
		$('.pie.border-color').css('borderColor', '#'+hexa);
	...
	$('.tara-timeline-year').not('.open').slideTo(158,300,'linear');
	...
	$('.tara-menuitem').waypoint(function(direction) {
		if (direction === 'down') {
			...
			var w = el.width();
			var p = el.position();
			jQuery('.nav li').removeClass('active'); jQuery('.nav li a').attr('style', '');
			$('#menu-line').stop().animate({'left': p.left, 'width': w}, 300);
			el.parent().addClass('active'); el.css('color', '#'+hexa);
		}
	...
	//	$('img.defaultimg').css('top', '-'+sia);
		...
		$('.tara-portfolio-filters a').click(function(){
			if ( !$(this).hasClass('selected') ) {
				...
				$this.addClass('selected').css('backgroundColor', '#'+hexa);
				var selector = $this.attr('data-filter');
				$container.isotope({ filter: selector }, function() {});
				$('.tara-portfolio-filters ul li a').unbind('mouseenter mouseleave');
				$('.tara-portfolio-filters ul li a').hover(function() {
					if ( !$(this).hasClass('selected') ) { 
						$(this).css('backgroundColor', '#'+hexa);	
					}
				}, function() {
					if ( !$(this).hasClass('selected') ) { 
						$(this).attr('style', '');
